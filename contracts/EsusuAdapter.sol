...
contract EsusuAdapter is OwnableService, ISavingsConfigSchema {
    ...
    */
    ...
    function UpdateVenusLendingService(address daiLendingServiceContractAddress)
        external
        onlyOwner
        active
    {
        _venusLendingService = IVenusLendingService(
    ...
    */
    ...
    function CreateEsusu(uint256 groupId, uint256 depositAmount, uint256 payoutIntervalSeconds,uint256 startTimeInSeconds, address owner, uint256 maxMembers) external active onlyOwnerAndServiceContract {
        ...
        //  emit event
        emit CreateEsusuCycleEvent(now, currentEsusuCycleId, depositAmount, owner, payoutIntervalSeconds,CurrencyEnum.BUSD,"BUSD Stablecoin",_esusuStorage.GetEsusuCycleState(currentEsusuCycleId));
    ...
    /*
        ...
        - Track the vBUSD shares that belong to this cycle using the derived equation below for save/investment operation
            - vBUSDSharesPerCycle = Change in vBUSDSharesForContract + Current vBUSD Shares in the cycle
            - Change in vBUSDSharesForContract = vBUSD.balanceOf(address(this) after save operation - vBUSD.balanceOf(address(this) after before operation
    */
    ...
    function JoinEsusu(uint esusuCycleId, address member) public onlyOwnerAndServiceContract active {
        ...
        uint256 memberBalance = _BUSD.balanceOf(member);
        ...
        _BUSD.safeTransferFrom(member, address(this), DepositAmount);
       ...
       _esusuStorage.CreateMemberAddressToMemberCycleMapping(
        ...
        //  Get the balance of vBUSDSharesForContract before save operation for this user
        uint vBUSDSharesForContractBeforeSave = _vBUSD.balanceOf(address(this));
        
        //  Invest the BUSD in Venus using Venus Lending Service.
        
        //  NOTE: vBUSD will be sent to this contract
        //  Transfer BUSD from this contract to Venus lending adapter and then call a new save function that will not use transferFrom internally
        //  Approve the VenusLendingAdapter so it can spend our BUSD on our behalf 
        address venusLendingAdapterContractAddress = _venusLendingService.GetVenusLendingAdapterAddress();
        _BUSD.approve(venusLendingAdapterContractAddress,DepositAmount);
        
        _venusLendingService.Save(DepositAmount);
        
        //  Get the balance of fBUSDSharesForContract after save operation
        uint vBUSDSharesForContractAfterSave = _vBUSD.balanceOf(address(this));
        
        
        //  Save fBUSD Total balanceShares for this member
        uint sharesForMember = vBUSDSharesForContractAfterSave.sub(vBUSDSharesForContractBeforeSave);
    ...
    */
    ...
    function GetBUSDBalance(address member)
        external
        view
        active
        returns (uint256)
    ...
    function GetVBUSDBalance(address member)
        external
        view
        active
        returns (uint256)
    {
        return _vBUSD.balanceOf(member);
    }
    ...
    */
    ...
    function TransferVBUSDSharesToWithdrawalDelegate(uint256 amount) external active onlyOwnerAndDelegateContract {
        
        _vBUSD.safeTransfer(_delegateContract, amount);
    }
    ...
    function DepricateContract(
        ...
        uint256 yDaiSharesBalance = _vBUSD.balanceOf(address(this));

        //  Send yDai shares to the new contract and halt operations of this contract
        _vBUSD.safeTransfer(newEsusuAdapterContract, yDaiSharesBalance);
